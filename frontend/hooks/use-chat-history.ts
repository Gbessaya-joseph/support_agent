...
interface ChatHistoryApiResponse {
    conversations: Array<{
        ...
        last_message_sender: "user" | "bot" | "admin";
...
export function useChatHistory() {
    ...
    const fetchConversations = useCallback(
        async (filter?: ConversationStatus) => {
            try {
                setLoading(true);
                setError(null);
                ...
                console.log(
                    "[useChatHistory] Response status:",
                    response.status,
                );
                ...
                const mappedConversations = data.conversations.map((conv) => ({
                    ...
                    lastMessageSender: conv.last_message_sender as
                        | "user"
                        | "bot"
                        | "admin",
                ...
                console.log(
                    "[useChatHistory] Fetched conversations:",
                    mappedConversations.length,
                );
                console.log(
                    "[useChatHistory] Conversation data:",
                    JSON.stringify(mappedConversations, null, 2),
                );
            } catch (err) {
                console.error(
                    "[useChatHistory] Error fetching conversations:",
                    err,
                );
                setError(
